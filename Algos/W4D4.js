...
function binarySearch(sortedNums, searchNum) {
    ...
    let middleElement = sortedNums[middleIndex];

    // base case
    if (middleElement === searchNum) return true;
    // recursive case
    else if (middleElement < searchNum && sortedNums.length > 1) {
        return binarySearch(sortedNums.slice(middleIndex, sortedNums.length), searchNum);
    }
    else if (middleElement > searchNum && sortedNums.length > 1) {
